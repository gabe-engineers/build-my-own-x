...
def aot_stage2_compile(aot_state: AOTState, aot_graph_capture: AOTGraphCapture, partition_fn: Callable, fw_compiler: Callable, bw_compiler: Callable | None = ..., inference_compiler: Callable | None = ...) -> DispatchReturn:
    ...
...
def collect_fw_donated_buffer_idxs(fw_ins: list[FakeTensor | None], user_fw_outs: list[FakeTensor | None], bw_outs: list[FakeTensor | None], saved_tensors: list[FakeTensor | None]) -> list[int]:
    """
...
def maybe_log_graph(gm: torch.fx.GraphModule, graph_name: str, aot_config: AOTConfig, structured_log_prefix_fn: Callable[[], str], out_structured_logs: list[str] | None = ...) -> None:
    ...
