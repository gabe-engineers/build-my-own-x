...
def reenter_make_fx(fn, subgraph_decomp_table=...): # -> _Wrapped[Callable[..., Any], Any, Callable[..., Any], GraphModule]:
    ...

def check_meta_consistency(lhs_list: list[torch.Tensor | torch.SymInt | int], rhs_list: list[torch.Tensor | torch.SymInt | int], lhs_name: str, rhs_name: str, include_contiguity: bool = ...) -> None:
    ...
...
def check_input_alias_and_mutation_return_outputs(gm: torch.fx.GraphModule) -> tuple[dict[int, int], dict[int, int], dict[int, int], list[int], tuple[Any, ...] | list[Any],]:
    ...
...
def call_op(op: OpOverload | HopInstance, args, kwargs): # -> Any:
    ...

def materialize_as_graph(fn: Callable, args: tuple[Any, ...], include_key_set: torch._C.DispatchKeySet | None = ..., exclude_key_set: torch._C.DispatchKeySet | None = ..., force_enable_grad=..., subgraph_decomp_table: Mapping[OpOverload, Callable] | None = ...) -> torch.fx.GraphModule:
    ...
